...
germany <- germany %>%
  ...
  mutate(locality = trimws(locality),
         ...
         diagnosticLab = case_when(is.na(diagnosticLab) ~ "Universities Trier and Braunschweig",
                                   TRUE ~ diagnosticLab),
         associatedReferences = case_when(is.na(associatedReferences) | associatedReferences == "" ~ "not specified",
  ...
  unite(., col = rowID, c(rowID_prefix, rowID_suffix), sep = "_") %>%
  ...
  mutate(materialSampleID = case_when(is.na(materialSampleID) ~ rowID,
...
df <- df %>%
  subset(., select = c(continentOcean, country, ADM1:ADM2, locality, decimalLatitude:decimalLongitude,
                       coordinateUncertaintyInMeters:georeferenceProtocol, locationID, yearCollected, monthCollected:dayCollected, verbatimEventDate,
...
spain <- spain %>%
  ...
  mutate(continent = "Europe",
   ...
   subset(., select = c(continent, country:Lon, coordinateUncertaintyInMeters, georeferenceProtocol,
...
china <- china %>%
  ...
  relocate(c(coordinateUncertaintyInMeters, georeferenceProtocol, locationID), .after = Lon) %>%
...
vietnam <- vietnam %>%
  ...
  unite(., materialSampleID, c(rowID_prefix, rowID_suffix), sep = "_", remove = T) %>%
  relocate(materialSampleID, .after = day) %>%
  subset(., select = c(continent, country, ADM1:ADM2, locality, Lat:Lon, coordinateUncertaintyInMeters:locationID,
...
adm1 <- data.frame(st_coordinates(st_cast(adm1_centroids$geometry, "POINT"))) %>%
  ...
  mutate(Lat = Y,
         ...
         country = COUNTRY,
         ADM1 = NAME_1) %>%
  subset(., select = c(Lat, Lon, ADM0, country, ADM1)) %>%
...
df <- df %>%
  ...
  relocate(c(continent, country, ADM0, ADM1:ADM4, locality), .before = LatLon) %>%
...
for(i in 1:nrow(df)){
  if(is.na(df[i, 8]) & is.na(df[i, 7]) & is.na(df[i, 6])){
    df[i, 8] <- paste(df[i, 5])}

  else if(is.na(df[i, 8]) & is.na(df[i, 7])){
    df[i, 8] <- paste(df[i, 6])}

  else if(is.na(df[i, 8])){
    df[i, 8] <- paste(df[i, 7])}

  else if(df[i, 8] != is.na(df[i, 8])){
    df[i, 8] <- paste(df[i, 8])}
}
...
df <- df %>%
  ...
  left_join(spr[,c(1:2, 62)], by = c("Site", "date"))
...
locs <- df %>%
  subset(., select = c(Lat, Lon, country, ADM0:locality, date, materialSampleID,
                       scientific, susceptibility, establishmentMeans)) %>%
  sf::st_as_sf(x = ., coords = c("Lon", "Lat"), crs = 4326, na.fail = F) %>%
  mutate(Lon = sf::st_coordinates(.)[,1],
         Lat = sf::st_coordinates(.)[,2]) %>%
  relocate(c(Lon, Lat), .before = country)

st_write(locs, file.path(shppath, "admData.gpkg"), append = F, delete_layer = T, layer = "adm_info")
...
gldas_daily <- read.csv(file.path(weatherpath, "weather_merged.csv"), header = T, encoding = "UTF-8")
...
tmin_df <- as.data.frame(tmin_extract) %>%
  ...
  relocate(c(id, continent, ADM0, country, ADM1:ADM4, Lon, Lat), .before = month)
...
bio_df <- as.data.frame(bio_extract) %>%
  mutate(bio12 = (0.1*bio12), # All bioclim precip vals are in mm. Convert to cm.
         ...
         id = row_number()) %>%
  left_join(., latlon_id, by = "id", keep = F) %>%
  relocate(c(id, continent, ADM0, country, ADM1:ADM4, Lon, Lat), .before = bio1)
...
wclim <- tmin_df %>%
  left_join(., tavg_df, by = c("id", "Lat", "Lon", "month", "continent", "country", "ADM0", "ADM1", "ADM2", "ADM3", "ADM4"), keep = F) %>%
  left_join(., tmax_df, by = c("id", "Lat", "Lon", "month", "continent", "country", "ADM0", "ADM1", "ADM2", "ADM3", "ADM4"), keep = F) %>%
  left_join(., prec_df, by = c("id", "Lat", "Lon", "month", "continent", "country", "ADM0", "ADM1", "ADM2", "ADM3", "ADM4"), keep = F) %>%
  left_join(., bio_df, by = c("id", "Lat", "Lon", "continent", "country", "ADM0", "ADM1", "ADM2", "ADM3", "ADM4")) %>%
  dplyr::select(-(id))
...
## 20 sites, initially Bsal-, were found to be Bsal+ sometime during these studies
df %>%
...
df <- df %>%
  ...
  mutate(georeferenceProtocol = "GPS") %>%
  subset(., select = -c(date_m2, date_m1, wk1_start, t4, year, month, day)) %>%
...
df <- df %>%
  mutate(specimenFate = case_when(is.na(specimenFate) ~ organismRemarks,
                                  TRUE ~ specimenFate),
         sex = case_when(is.na(sex) |sex == "V" | sex == "" ~ "not determined",
                         sex == "m" | sex == "M" | sex == "M " ~ "male",
                         sex == "F" ~ "female",
                         TRUE ~ sex)) %>%
  rename(specimenDisposition = specimenFate) %>%
  relocate(c(posSite, Site), .after = locality) %>%
  relocate(c(date, dateFirstPositive), .after = Lon) %>%
  relocate(c(individualCount:specimenDisposition, lifeStage, sex), .after = susceptibility) %>%
  relocate(diagnosticLab, .after = collectorList) %>%
  relocate(dataConfidence, .after = associatedReferences) %>%
  subset(., select = -c(coordinateUncertaintyInMeters:georeferenceProtocol,
                        assessmentScope, diseaseDetected:whichDisease,
                        sampleType:testMethod, basisOfRecord, organismRemarks))
