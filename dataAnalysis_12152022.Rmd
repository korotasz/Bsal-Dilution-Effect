...
# Scale data:
scaled_cbind <- dcbind_alpestris
scaled_cbind$temp_date <- scale(scaled_cbind$temp_date)
scaled_cbind$soilMoisture_date <- scale(scaled_cbind$soilMoisture_date)

# Save scaled attributes:
scaleList <- scaled_cbind %>%
  list(temp_scale = as.numeric(attr(scaled_cbind$temp_date, "scaled:scale")),
    temp_center = as.numeric(attr(scaled_cbind$temp_date, "scaled:center")),
    soilMoisture_scale = as.numeric(attr(scaled_cbind$soilMoisture_date, "scaled:scale")),
    soilMoisture_center = as.numeric(attr(scaled_cbind$soilMoisture_date, "scaled:center")))

m3a_scaled <- glmmTMB(cbind(YesBsal, NoBsal) ~  logsiteAbun*richness + temp_date*soilMoisture_date + (1|scientific),
               data = scaled_cbind, family = "binomial",
               control = glmmTMBControl(optimizer = optim, 
                                        optArgs = list(method = "BFGS")))
...
temp_scaled <- scale(dcbind_alpestris$temp_date)
attributes(temp_scaled)
test_bcktrnsfm <- temp_scaled * attr(temp_scaled, 'scaled:scale') + attr(temp_scaled, 'scaled:center')
