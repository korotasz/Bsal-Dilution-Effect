...
dcbind_og <- read.csv("bsalData_cbind_og.csv")
dcbind_new <- read.csv("bsalData_cbind.csv", header = T, encoding = "UTF-8")
...
# Scale and log transform relevant vars
dcbindScaled_og <- dcbind_og %>%
  mutate(logsppAbun = log(sppAbun),
         logsiteAbun = log(siteAbun),
         scientific = as.factor(scientific),
         susceptibility = as.factor(susceptibility)) %>%
  relocate(c(logsppAbun, logsiteAbun), .after = sppAbun) %>%
  mutate_at(c("temp_d", "temp_m", "temp_m1", "temp_m2",
              "sMoist_d", "sMoist_m", "sMoist_m1", "sMoist_m2",
              "precip_m", "precip_m1", "precip_m2",
              "bio1_wc", "bio12_wc", "tavg_wc", "prec_wc"), 
            ~(scale(., center = T, scale = T %>% as.numeric))) %>%
    tidyr::drop_na(., any_of(c(21:41))) %>%
    filter(country == "Germany" | country == "Spain")


dcbindScaled_new <- dcbind_new %>%
  mutate(logsppAbun = log(sppAbun),
         logsiteAbun = log(siteAbun),
         scientific = as.factor(scientific),
         susceptibility = as.factor(susceptibility)) %>%
  ...
  mutate_at(c("temp_d", "temp_m", "temp_m1", "temp_m2",
              ...
              "bio1_wc", "bio12_wc", "tavg_wc", "prec_wc"), 
            ~(scale(., center = T, scale = T %>% as.numeric))) %>%
  tidyr::drop_na(., any_of(c(26:46)))

dcbindScaled_noFS <- dcbind_new %>%
...
Europe polygons were re-projected to EPSG:27704 (WGS84/Equi7 Europe) for mapping purposes. These projections were made as a part of a larger project to optimize handling remote-sensing data, with each continent having their own centroid.
paper doi: https://doi.org/10.1016/j.cageo.2014.07.005
Equi7Grid GitHub: https://github.com/TUW-GEO/Equi7Grid

```{figuresSetup, echo = FALSE}
...
```{r staticMap, fig.width = 24, fig.height = 12, fig.fullwidth = TRUE, fig.fullheight = TRUE, echo = FALSE}
# Summarise number of observations from each country
obs <- d %>%
  filter(prev_above_0 == "1") %>%
  dplyr::select(country, ADM0, Lat, Lon, diseaseTested, prev_above_0,
                BsalDetected, BdDetected, individualCount) %>%
  plyr::mutate(BsalDetected = as.factor(dplyr::recode(prev_above_0,
                                                      "1" = "Bsal+ Site"))) %>%
  arrange(BsalDetected)

# plotting individual points on map
obs_transformed <- st_as_sf(obs, coords = c("Lon", "Lat"), crs = 4326) %>%
...
europe_map <- ggplot() +
  ...
  geom_sf(data = obs_transformed, aes(geometry = geometry, fill = BsalDetected, shape = BsalDetected),
          alpha = 0.3, size = 4, stroke = 1, color = "gray40", show.legend = "point") +
  geom_sf_text(data = countriesSampled, aes(label = name), position = "identity", size = 5) +
  scale_fill_manual(values = c("#b30000"), guide = "none") +
  ...
  ak_theme + theme(legend.title = element_blank(),
                   ...
                   axis.text.y = element_text(size = 28, face = "plain"),
                   axis.title.y = element_blank()) +
  guides(fill = guide_legend(override.aes = list(color = c("#b30000"),
                                                 shape = c(21), 
                                                 size = c(5),
                                                 alpha = c(1))))
...
deu_map <- ggplot() +
  geom_sf(data = europe, col = "gray40", fill = "#ECECEC", show.legend = F) +
  geom_sf(data = countriesSampled, aes(fill = sovereignt), col = "gray40", fill = "#B2BEB5", show.legend = F) +
  geom_sf_label(data = g, aes(label = paste(label)), nudge_x = -130000, nudge_y = 400000,
                size = 7, fontface = "bold", label.size = NA, alpha = 0.5) +
  geom_sf(data = obs_transformed, aes(geometry = geometry, fill = BsalDetected, shape = BsalDetected),
          alpha = 0.3, size = 4, stroke = 1, color = "gray40", show.legend = "point") +
  scale_fill_manual(values = c("#b30000"), guide = "none") +
...
esp_map <- ggplot() +
  geom_sf(data = europe, col = "gray40", fill = "#ECECEC", show.legend = F) +
  geom_sf(data = countriesSampled, aes(fill = sovereignt), col = "gray40", fill = "#B2BEB5", show.legend = F) +
  geom_sf_label(data = s, aes(label = paste(label)), nudge_x = -170000, nudge_y = 520000,
                 size = 7, fontface = "bold", label.size = NA, alpha = 0.5) +
  geom_sf(data = obs_transformed, aes(geometry = geometry, fill = BsalDetected, shape = BsalDetected),
          alpha = 0.3, size = 4, stroke = 1, color = "gray40", show.legend = "point") +
  scale_fill_manual(values = c("#b30000"), guide = "none") +
  scale_shape_manual(values = 24, guide = "none") +
...
fig1a <- europe_map + theme(plot.tag.position = c(0.90, 0.88)) 
fig1b <- esp_map + theme(plot.tag.position = c(0.95, 0.77)) 
fig1c <- deu_map + theme(plot.tag.position = c(0.96, 0.94)) 
...
## 'All species' INCLUDING FIRE SALAMANDERS cbind model - ORIGINAL
Original .csv file. Upon further examination, Bsal pos & Bsal neg sites were both used in this model. I included all data from Germany and Spain.
```{r allsppModel, eval = TRUE}
m_all <- glmmTMB(cbind(nPos_all, nNeg_all) ~ richness*logsiteAbun +
                    temp_d*sMoist_d + (1|scientific),
                  data = dcbindScaled_og, family = "binomial", na.action = "na.fail",
                  control = glmmTMBControl(optimizer = optim,
                                           optArgs = list(method = "BFGS")))
...
m_all_plot <- ggplot(m_all_predict, aes(x = richness, y = predicted, 
  ...
  geom_rug(data = dcbindScaled_og, aes(x = richness, y = 0), sides = "b", 
...
## 'All species' INCLUDING FIRE SALAMANDERS cbind model
ONLY Bsal positive sites used in this model

```{r allsppModel_new, eval = TRUE}
m_all_new <- glmmTMB(cbind(nPos_all, nNeg_all) ~ richness*logsiteAbun +
                    temp_d*sMoist_d + (1|scientific),
                  data = dcbindScaled_new, family = "binomial", na.action = "na.fail",
                  control = glmmTMBControl(optimizer = optim,
                                           optArgs = list(method = "BFGS")))
...
```{r m_all_newPlot, fig.width = 12, fig.height = 7, fig.fullwidth = TRUE, fig.fullheight = TRUE, echo = FALSE, eval = TRUE}
##      Prevalence by Abundance & Richness Plots for 'All spp.' model
m_all_predict <- ggpredict(m_all_new,  terms = c("richness", "logsiteAbun")) %>%
  dplyr::rename("richness" = "x",
         "logsiteAbun" = "group") %>%
  plyr::mutate(richness = as.numeric(as.character(richness)),
               logsiteAbun = as.numeric(as.character(logsiteAbun)),
...
m_all_plot <- ggplot(m_all_predict, aes(x = richness, y = predicted, 
                                        colour = siteAbun)) +
  geom_line(aes(linetype = siteAbun), linewidth = 1) +
  geom_rug(data = dcbindScaled_noFS, aes(x = richness, y = 0), sides = "b", 
           alpha = 0.5, position = position_jitter(width = 0.4, height = 0.1), 
           inherit.aes = F, na.rm = T) +
  labs(x = "Species richness",
       y = "Bsal prevalence (%)",
...
## 'All species' EXCLUDING FIRE SALAMANDERS cbind model - ORIGINAL

Original .csv file. Upon further examination, Bsal pos & Bsal neg sites were both used in this model. I included all data from Germany and Spain.
```{r noFS_model, eval = TRUE}
noFS <- dcbindScaled_og %>%
  filter(scientific != "Salamandra salamandra")

m_noFS <- glmmTMB(cbind(nPos_all, nNeg_all) ~ richness*logsiteAbun +
                    temp_d*sMoist_d + (1|scientific),
                  data = dcbindScaled_og, family = "binomial", na.action = "na.fail",
                  control = glmmTMBControl(optimizer = optim,
                                           optArgs = list(method = "BFGS")))
...
```{r noFS_allPlot, fig.width = 12, fig.height = 7, fig.fullwidth = TRUE, fig.fullheight = TRUE, echo = FALSE, eval = TRUE}
##      Prevalence by Abundance & Richness Plots for 'All spp.' model
m_all_predict <- ggpredict(m_all,  terms = c("richness", "logsiteAbun")) %>%
  dplyr::rename("richness" = "x",
         "logsiteAbun" = "group") %>%
...
## 'All species' EXCLUDING FIRE SALAMANDERS cbind model
FS included in richness, site abundance, and species abundance calculations, as well as when determining which sites were Bsal positive, but they were excluded when calculating 'nPos_all' and 'nNeg_all'. ONLY Bsal positive sites used in this model.

```{r allsppModel_noFS, eval = TRUE}
m_all_noFS <- glmmTMB(cbind(nPos_all_noFS, nNeg_all_noFS) ~ richness*logsiteAbun +
                    temp_d*sMoist_d + (1|scientific),
                  data = dcbindScaled_new, family = "binomial", na.action = "na.fail",
                  control = glmmTMBControl(optimizer = optim,
                                           optArgs = list(method = "BFGS")))
...
```{r m_all_noFSPlot, fig.width = 12, fig.height = 7, fig.fullwidth = TRUE, fig.fullheight = TRUE, echo = FALSE, eval = TRUE}
##      Prevalence by Abundance & Richness Plots for 'All spp.' model
m_all_predict <- ggpredict(m_all_noFS,  terms = c("richness", "logsiteAbun")) %>%
  dplyr::rename("richness" = "x",
         "logsiteAbun" = "group") %>%
  plyr::mutate(richness = as.numeric(as.character(richness)),
               logsiteAbun = as.numeric(as.character(logsiteAbun)),
